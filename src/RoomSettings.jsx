...
import { setRoomSlowMode, setRoomBlockedWords, addModerator, removeModerator, store } from './db'
...
function RoomSettings({ room, onClose }) {
  ...
  async function handleAddMod() {
    ...
    setSaving(true)
    try {
      // Verify the UID corresponds to a real user before adding them as mod
      const userSnap = await getDoc(doc(store, 'users', uid))
      if (!userSnap.exists()) {
        setModError('User not found. Check the UID and try again.')
        return
      }
      ...
      setModError('Failed to add moderator. Please try again.')
    } finally {
      setSaving(false)
    }
  }
